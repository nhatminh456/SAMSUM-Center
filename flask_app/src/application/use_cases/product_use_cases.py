...
class CreateProductUseCase:
    ...
    def execute(self, product: Product) -> tuple[bool, str, Optional[int]]:
        ...
        if product.category_id <= 0:
            return False, "Danh mục sản phẩm không hợp lệ", None
        ...
        error_detail = ""
        if hasattr(self.product_repository, "get_last_error"):
            error_detail = self.product_repository.get_last_error()

        if error_detail:
            return False, f"Đã xảy ra lỗi khi tạo sản phẩm: {error_detail}", None
...
class DeleteProductUseCase:
    ...
    def execute(self, product_id: int) -> tuple[bool, str]:
        ...
        # MySQL FK constraint error when product exists in order_items
        if "1451" in error_detail:
            return False, "Không thể xóa sản phẩm vì đã tồn tại trong đơn hàng"

        if error_detail:
            return False, f"Đã xảy ra lỗi khi xóa: {error_detail}"
